...
for (i in 1:2) {
 results_vec <- purrr::map(1:niter, ~ prediction(proj_score1 = wc_iter[i, ] |> pull(proj_score1), proj_score2 = wc_iter[i, ] |> pull(proj_score2))) |>
  unlist()
 
 results_mat[i] <- tibble(outcome = results_vec) |> group_by(outcome) |>
  summarise(total = n()) |>
