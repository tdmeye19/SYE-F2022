...
wc_iter <- wc_matches |> select(proj_score1, proj_score2, everything())
wc_iter

niter <- 100

results_mat <- matrix(NA, nrow = nrow(wc_iter), ncol = 3)


results_vec <- purrr::map(1:niter, ~ prediction(proj_score1 = wc_iter[1, ] |> pull(proj_score1), proj_score2 = wc_iter[1, ] |> pull(proj_score2))) |>
  unlist()
results_mat[1, ] <- tibble(outcome = results_vec) |> group_by(outcome) |>
  summarise(total = n()) |>
...
for (i in 1:3) {
 print(purrr::map2(1:5, ~ prediction(proj_scores$proj_score1[i], proj_scores$proj_score2[i])))
}
