...
|             I would like to thank Dr. Karin Heckman for her assistance during the independent study portion, which included gaining IRB approval and administering both surveys. This assistance allowed me to understand how the IRB process works, as well as how to create and administer surveys using Qualtrics.

|             I would like to thank Dr. Matt Higham for his assistance during the SLU Fellowship, specifically in helping answer any questions related to the statistical analysis. His patience is greatly valued while I learned how to create visualizations and manipulate data in R, as well as make posters, presentations, and reports using R Markdown.
...
locationplot22 <- ggplot(data = location_df22_top, 
         aes(x = fct_reorder(location, totalrespondents), y = prop)) +
  geom_point(size = 5) + 
...
locationplot21 <- ggplot(data = location_df21_top, 
         aes(x = fct_reorder(location, totalrespondents), y = prop)) +
  geom_point(size = 5) + 
  geom_segment(aes(x=location, xend=location, y=0, yend=prop), size = 1.5) +
  coord_flip() +
...
(ggplot(data = wc_long_iter_total, 
        aes(x = proptimes, y = fct_reorder(team, proptimes))) +
  geom_point() + 
  geom_segment(data = wc_long_iter_total, 
               aes(x = 0, xend = proptimes, y = team, yend = team)) +
  geom_segment(data = wc_long_18_through, 
               aes(x = 0, xend = proptimes, y = team, yend = team), colour = "red") +
...
(ggplot(data = wc_long_iter_total_22, 
        aes(x = proptimes, y = fct_reorder(team, proptimes))) +
  geom_point() + 
  geom_segment(data = wc_long_iter_total_22, 
               aes(x = 0, xend = proptimes, y = team, yend = team)) + 
  geom_segment(data = wc_long_22_through, 
               aes(x = 0, xend = proptimes, y = team, yend = team), colour = "red") +
